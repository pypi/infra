...
resource "fastly_service_vcl" "pypi" {
  ...
  # Edge-only 5xx (fastly.ff guard avoids shield double-logging)
  condition {
    name      = "Edge 5xx Error"
    type      = "RESPONSE"
    statement = "(resp.status >= 500 && resp.status < 600) && fastly.ff.visits_this_service == 0"
  }
