...
resource "fastly_service_vcl" "pypi" {
  ...
  logging_datadog {
    ...
    response_condition = "5xx Error"
    format             = "{ \"ddsource\": \"fastly\", \"status\": \"%%{if(resp.status >= 500, \"error\", if(resp.status >= 400, \"warning\", \"info\"))}V\", \"service\": \"%%{req.service_id}V\", \"date\": \"%%{begin:%Y-%m-%dT%H:%M:%S%z}t\", \"origin_status\": \"%%{req.http.X-Origin-Status}V\", \"status_code\": \"%%{resp.status}V\", \"method\": \"%%{req.method}V\", \"url\": \"%%{json.escape(cstr_escape(req.url))}V\", \"host\": \"%%{json.escape(if(req.http.Fastly-Orig-Host, req.http.Fastly-Orig-Host, req.http.Host))}V\", \"referer\": \"%%{json.escape(req.http.referer)}V\", \"useragent\": \"%%{json.escape(req.http.User-Agent)}V\", \"client_ip\": \"%%{if(req.http.Fastly-Client-IP, req.http.Fastly-Client-IP, client.ip)}V\", \"geo_country\": \"%%{client.geo.country_code}V\", \"geo_city\": \"%%{json.escape(client.geo.city.utf8)}V\", \"geo_region\": \"%%{client.geo.region}V\", \"geo_proxy_type\": \"%%{client.geo.proxy_type}V\", \"geo_proxy_description\": \"%%{json.escape(client.geo.proxy_description)}V\", \"request_time_ms\": \"%%{time.elapsed.msec}V\", \"fastly_state\": \"%%{fastly_info.state}V\", \"node\": \"%%{if(req.http.Fastly-FF, \"shield\", \"edge\")}V\", \"pop\": \"%%{server.datacenter}V\", \"restarts\": \"%%{req.restarts}V\", \"backend\": \"%%{req.backend}V\", \"protocol\": \"%%{req.protocol}V\", \"tls_version\": \"%%{tls.client.protocol}V\", \"resp_bytes\": \"%%{resp.http.Content-Length}V\", \"x_cache\": \"%%{resp.http.X-Cache}V\" }"
  }
